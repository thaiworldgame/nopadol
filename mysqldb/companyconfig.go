...
func (repo *configRepository) SearchById(req *companyconfig.SearchByIdRequestTemplate) (resp interface{}, err error) {
	...
	sql := `select a.id,com_sys_id,a.company_name,ifnull(a.name_eng,'') as name_eng,ifnull(a.address,'') as address,ifnull(a.telephone,'') as telephone,ifnull(a.fax,'') as fax,ifnull(a.tax_number,'') as tax_number,ifnull(email,'') as email,ifnull(a.web_site,'') as web_site,a.tax_rate,ifnull(b.branch_name,'') as branch_name,ifnull(b.address,'') as branch_address,ifnull(b.telephone,'') as branch_telephone,b.stock_status,b.sale_tax_type,b.buy_tax_type,ifnull(b.def_sale_wh,'') as def_sale_wh,ifnull(b.def_sale_shelf,'') as def_sale_shelf,ifnull(b.def_buy_wh,'') as def_buy_wh,ifnull(b.def_buy_shelf,'') as def_buy_shelf,ifnull(b.logo_path,'') as logo_path from Company a inner join Branch b on a.id = b.company_id where b.id = ? and active_status = 1`
