...
func (d *deliveryRepository) ReportDailyByTeam(req *delivery.ReportDORequestByTeam) (interface{}, error) {
	log.Println("begin postgres.ReportDaily")
	fmt.Println("param req -> ", req)
	type doModel struct {
		id          sql.NullInt64
	...
	lccommand := "select * from sm_do.doreport_by_team('" + req.Date + "' , '" + req.Profit + "')"
	//lccommand = "select * from sm_do.doreport('2018-07-06')"
	...
	for rs.Next() {
		err := rs.Scan(&_do.id, &_do.doDocno, &_do.soNo, &_do.confirmDate,
			&_do.doDate, &_do.diffDate, &_do.description, &_do.arName,
			&_do.itemAmount, &_do.itemGroup, &_do.remark, &_do.invoiceno,
			&_do.carlicense, &_do.salecode, &_do.saleman, &_do.profit)
		if err != nil {
			return nil, err
		}
...
func (d *deliveryRepository) GetTeam() (interface{}, error) {
	lcCommand := "	select distinct profitcenter as profit from sm_do.saleteam where COALESCE(upper(profitcenter),'') <> '' " +
	...
	type teamModel struct {
		ProfitCenter string `db:"profit" json:"profit"`
	}
	t := teamModel{}
	Ts := []teamModel{}
...
func (d *deliveryRepository) GetSale(_profit string) (interface{}, error) {
	...
	lcCommand := "select distinct salecode,salename from sm_do.saleteam where COALESCE(profitcenter,'') = '" + _profit + "'" +
		" and activestatus=1 and COALESCE(profitcenter,'') <> '' and salecode in " +
		" (  select distinct salecode from sm_do.tb_do_queue where  date_trunc('year',createdate)=date_trunc('year',now()) )" +
		" order by salecode"
